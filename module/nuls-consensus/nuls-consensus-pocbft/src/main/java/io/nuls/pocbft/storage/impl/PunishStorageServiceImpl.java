...
import io.nuls.pocbft.storage.PunishStorageService;
...
public class PunishStorageServiceImpl implements PunishStorageService {
    ...
    public boolean save(PunishLogPo po,int chainID) {
        ...
        try {
            return  RocksDBService.put(ConsensusConstant.DB_NAME_PUNISH+chainID,po.getKey(),po.serialize());
        }catch (Exception  e){
            Log.error(e);
            return false;
        }
    ...
    public boolean delete(byte[] key,int chainID) {
        ...
        try {
            return RocksDBService.delete(ConsensusConstant.DB_NAME_PUNISH+chainID,key);
        }catch (Exception e){
            Log.error(e);
            return false;
        }
    ...
    public List<PunishLogPo> getPunishList(int chainID)throws Exception{
        try {
            List<Entry<byte[], byte[]>> list = RocksDBService.entryList(ConsensusConstant.DB_NAME_PUNISH+chainID);
