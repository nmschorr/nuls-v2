...
import io.nuls.pocbft.model.bo.round.RoundValidResult;
...
import io.nuls.pocbft.utils.compare.CoinFromComparator;
import io.nuls.pocbft.utils.compare.CoinToComparator;
import io.nuls.pocbft.utils.enumeration.PunishReasonEnum;
...
public class BlockValidator {
     ...
     */
    public void validate(Chain chain, Block block) throws NulsException, IOException {
        ...
        validResult = coinBaseValidate(block, member, chain, blockHeaderHash);
        if (!validResult) {
            if (roundValidResult.isValidResult()) {
                roundManager.rollBackRound(chain, currentRound.getIndex());
            }
     ...
     */
    private RoundValidResult roundValidate(Chain chain, BlockHeader blockHeader, String blockHeaderHash) throws Exception {
        BlockExtendsData extendsData = blockHeader.getExtendsData();
        ...
        * */
        boolean isBeforeBlock = extendsData.getRoundIndex() < bestExtendsData.getRoundIndex()
                ...
                || blockHeader.getTime() < NulsDateUtils.getCurrentTimeSeconds();
        if (isBeforeBlock) {
        ...
        if (currentRound == null || extendsData.getRoundIndex() < currentRound.getIndex()) {
            ...
            MeetingRound tempRound = roundManager.getRound(chain, extendsData, false);
     ...
     * @param chain        chain info
     */
    private boolean coinBaseValidate(Block block, MeetingMember member, Chain chain, String blockHeaderHash) throws NulsException, IOException{
        ...
        try {
            coinBaseTransaction = ConsensusManager.createCoinBaseTx(chain, member, block.getTxs(),  block.getHeader().getTime(), false);
        }catch (Exception e){
