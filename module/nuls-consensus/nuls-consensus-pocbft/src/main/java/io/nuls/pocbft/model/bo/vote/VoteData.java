...
public class VoteData {
    ...
    public boolean isRepeatMessage(byte voteRound,byte voteStage, String address){
        return !voteRoundMap.get(voteRound).getVoteStageDate(voteStage).getHaveVotedAccountSet().add(address);
    }
     ...
     * 设置指定指定轮次指定阶段投票结果数据
     * */
    public void setVoteResult(byte voteRound, byte voteStage, VoteResultData voteResultData){
        ...
        if(voteStage == ConsensusConstant.VOTE_STAGE_TWO){
            ...
            if(voteResultData.isResultSuccess()){
               setFinished(true);
               voteRoundMap.put(ConsensusConstant.FINAL_VOTE_ROUND_SIGN, voteRoundMap.get(voteRound));
            }
        }
    }
}
