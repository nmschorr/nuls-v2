...
import io.nuls.pocbft.utils.validator.CreateAgentValidator;
...
public class CreateAgentProcessor implements TransactionProcessor {
    ...
    public boolean commit(int chainId, List<Transaction> txs, BlockHeader blockHeader) {
        ...
        if (!commitResult) {
            for (Transaction rollbackTx : commitSuccessList) {
                createAgentRollBack(rollbackTx, chain);
            }
        }
        return commitResult;
    }
    ...
    public boolean rollback(int chainId, List<Transaction> txs, BlockHeader blockHeader) {
        ...
        for (Transaction tx : txs) {
            if (createAgentRollBack(tx, chain)) {
    ...
    private boolean createAgentCommit(Transaction transaction, BlockHeader blockHeader, Chain chain) {
        ...
        return agentManager.addAgent(chain, agent);
    }

    private boolean createAgentRollBack(Transaction transaction, Chain chain) {
        return  agentManager.removeAgent(chain, transaction.getHash());
    }
}
