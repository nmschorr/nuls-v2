...
public class BlockCmd extends BaseCmd {
     ...
     * 缓存新区块头
     * */
    @CmdAnnotation(cmd = CMD_ADD_BLOCK, version = 1.0, description = "接收并缓存新区块/Receiving and caching new blocks")
     ...
     * 验证区块正确性
     * */
    @CmdAnnotation(cmd = CMD_VALID_BLOCK, version = 1.0, description = "验证区块/verify block correctness")
    @Parameter(parameterName = PARAM_CHAIN_ID, requestType = @TypeDescriptor(value = int.class), parameterDes = "链id")
     ...
     * 区块回滚
     * */
    @CmdAnnotation(cmd = CMD_CHAIN_ROLLBACK, version = 1.0, description = "区块回滚/chain rollback")
    @Parameter(parameterName = PARAM_CHAIN_ID, requestType = @TypeDescriptor(value = int.class),parameterDes = "链id")
