...
public class AgentStorageServiceImpl implements AgentStorageService{
    ...
    public boolean save(AgentPo agentPo,int chainID) {
        ...
        try {
            ...
            return RocksDBService.put(ConsensusConstant.DB_NAME_AGENT+chainID,key,value);
        }catch (Exception e){
            Log.error(e);
            return  false;
        }
    ...
    public AgentPo get(NulsHash hash,int chainID) {
        ...
        try {
            ...
            byte[] value = RocksDBService.get(ConsensusConstant.DB_NAME_AGENT+chainID,key);
            if(value == null){
                return  null;
            }
    ...
    public boolean delete(NulsHash hash,int chainID) {
        ...
        try {
            byte[] key = hash.getBytes();
            return RocksDBService.delete(ConsensusConstant.DB_NAME_AGENT+chainID,key);
        }catch (Exception e){
            Log.error(e);
            return  false;
        }
    ...
    public List<AgentPo> getList(int chainID) throws NulsException {
            List<Entry<byte[], byte[]>> list = RocksDBService.entryList(ConsensusConstant.DB_NAME_AGENT+chainID);
    ...
    public int size(int chainID) {
        List<byte[]> keyList = RocksDBService.keyList(ConsensusConstant.DB_NAME_AGENT+chainID);
        if(keyList != null){
            return keyList.size();
        }
        return 0;
