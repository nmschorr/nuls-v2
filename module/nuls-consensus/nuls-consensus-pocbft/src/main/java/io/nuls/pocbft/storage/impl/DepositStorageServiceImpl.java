...
public class DepositStorageServiceImpl implements DepositStorageService {
    ...
    public boolean save(DepositPo depositPo, int chainID) {
        ...
        try {
            ...
            return RocksDBService.put(ConsensusConstant.DB_NAME_DEPOSIT + chainID, key, value);
        } catch (Exception e) {
            Log.error(e);
            return false;
        }
    ...
    public DepositPo get(NulsHash hash, int chainID) {
        ...
        try {
            byte[] value = RocksDBService.get(ConsensusConstant.DB_NAME_DEPOSIT + chainID, hash.getBytes());
            if (value == null) {
                return null;
            }
    ...
    public boolean delete(NulsHash hash, int chainID) {
        ...
        try {
            ...
            return RocksDBService.delete(ConsensusConstant.DB_NAME_DEPOSIT + chainID, key);
        } catch (Exception e) {
            Log.error(e);
            return false;
        }
    ...
    public List<DepositPo> getList(int chainID) throws NulsException {
        List<Entry<byte[], byte[]>> list = RocksDBService.entryList(ConsensusConstant.DB_NAME_DEPOSIT + chainID);
    ...
    public int size(int chainID) {
        List<byte[]> keyList = RocksDBService.keyList(ConsensusConstant.DB_NAME_DEPOSIT + chainID);
        if (keyList != null) {
            return keyList.size();
        }
        return 0;
